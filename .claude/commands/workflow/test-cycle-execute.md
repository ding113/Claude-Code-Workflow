...
- **@cli-planning-agent**: Executes CLI analysis (Gemini), parses results, generates fix task TOON files
...
| Run CLI analysis (Gemini) | No - Delegates to cli-planning-agent | Yes - Executes CLI internally | No - Not involved |
...
│   ├── [@cli-planning-agent] Execute CLI tool (Gemini) internally
...
```json
{
  ...
  "context": {
    ...
    "fix_strategy": {
      "approach": "Generated by CLI tool (Gemini) analysis",
...
6. **[@cli-planning-agent]** Executes internally:
   - Runs Gemini CLI analysis with bug diagnosis template
...
Task(
  ...
  prompt=`
    ...
    ## Your Task
    1. Execute CLI analysis using Gemini (fallback to Gemini if needed)
...
When @cli-planning-agent returns `status: "degraded"` (both Gemini failed):
